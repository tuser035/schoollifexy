      ...
      <main className="container mx-auto px-3 sm:px-4 py-4 sm:py-8">
        ...
        <Tabs value={activeGroup} onValueChange={handleGroupChange} className="w-full mb-4">
          <div className="overflow-x-auto [&::-webkit-scrollbar]:h-1 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-border [&::-webkit-scrollbar-thumb]:rounded-full">
            <TabsList className="flex w-full bg-muted/50 rounded-lg p-1">
              {tabGroups.map((group) => {
                ...
                return (
                  <TabsTrigger
                    key={group.name}
                    value={group.name}
                    className={`flex-1 whitespace-nowrap text-xs sm:text-sm px-1 sm:px-3 py-2 font-medium rounded-md ${firstItem.activeClass}`}
                  >
                    {group.label}
                  </TabsTrigger>
                );
        ...
        {currentGroup && (
          <div className="mb-4 sm:mb-6">
            {/* 그룹 타이틀 */}
            <h2 className={`text-lg sm:text-xl font-bold mb-3 ${currentGroup.items[0]?.textClass || 'text-foreground'}`}>
              {currentGroup.label}
            </h2>
                    ...
                    {currentGroup.items.map((item) => {
                      ...
                      return (
                        <TabsTrigger
                          key={item.value}
                          value={item.value}
                          className={`flex-1 whitespace-nowrap text-xs sm:text-sm px-1 sm:px-2 rounded-md ${item.activeClass}`}
                        >
