-- Drop and recreate admin_get_students function to include parent phone numbers
DROP FUNCTION IF EXISTS public.admin_get_students(uuid, text, integer, integer);
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(s.parents_call1, '-') as parents_call1,
    COALESCE(s.parents_call2, '-') as parents_call2
