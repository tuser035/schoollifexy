...
### 💻 상담 조회 (CounselingInquiry) 실시간 동기화 코드

상담 조회 컴포넌트는 `career_counseling` 테이블 하나만 구독하면 됩니다.
다른 컴포넌트들과 달리 **단일 테이블**만 감시하므로 코드가 더 간단합니다!
...
}, [records.length, searchInput]);  // searchInput도 의존성에 포함
```

**쉽게 이해하기:**
- `records`: 현재 조회된 상담 기록 배열
- `searchInput`: 검색어 (학번, 이름, 학년반)

---

#### Step 2: career_counseling 테이블 실시간 구독
...
useEffect(() => {
  const counselingChannel = supabase
    ...
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'career_counseling' },  // 상담 테이블
      (payload) => {
...
**쉽게 이해하기:**
- 상담 조회는 `career_counseling` 테이블 하나만 감시합니다
- 다른 관리자가 상담 기록을 추가/수정/삭제하면 자동으로 목록이 갱신됩니다
- 코드가 단순해서 이해하기 쉽습니다!

---

### 🤔 단일 테이블 vs 다중 테이블 구독 비교

| 컴포넌트 | 구독 테이블 수 | 이유 |
|----------|----------------|------|
| **CounselingInquiry** | 1개 (career_counseling) | 상담 기록만 표시 |
| **PointsInquiry** | 3개 (merits, demerits, monthly) | 상벌점 합계 계산 필요 |
| **DataInquiry** | 5개 (students, teachers, merits, demerits, monthly) | 다양한 데이터 유형 조회 |
| **StudentLeaderboard** | 3개 (merits, demerits, monthly) | 순위 계산에 모두 필요 |

```
단일 테이블 구독 (간단!)       다중 테이블 구독 (복잡!)
┌──────────────────┐          ┌──────────────────┐
│ career_counseling │          │   merits         │
│        │         │          │      │           │
│        ▼         │          ├──────┼───────────┤
│   handleQuery()  │          │ demerits         │
└──────────────────┘          │      │           │
                              ├──────┼───────────┤
                              │  monthly         │
                              │      │           │
...
### 🧪 과제 10: 단일 테이블 실시간 구독 코드 작성하기

`email_history` 테이블 변경을 감지하는 실시간 구독 코드를 작성해보세요.
(힌트: CounselingInquiry와 동일한 패턴입니다)
...
useEffect(() => {
  const emailChannel = supabase
    ...
    .on(
      ...
      { event: '*', schema: 'public', table: 'email_history' },
      (payload) => {
        console.log('Email history changed:', payload);
...
### 💻 이메일 히스토리 (EmailHistory) 실시간 동기화 코드

이메일 히스토리 컴포넌트는 `email_history` 테이블 하나만 구독합니다.
CounselingInquiry와 마찬가지로 **단일 테이블 구독** 패턴입니다.
...
**쉽게 이해하기:**
- `history`: 현재 조회된 이메일 이력 배열
- 검색 조건이 바뀌면 이벤트 리스너도 다시 설정됩니다

---

#### Step 2: email_history 테이블 실시간 구독
...
**쉽게 이해하기:**
- 다른 교사나 관리자가 이메일을 발송하면 자동으로 목록이 갱신됩니다
- INSERT와 UPDATE만 처리합니다 (이메일은 보통 삭제하지 않음)
...
| 컴포넌트 | 위치 | 구독 테이블 | 특징 |
|----------|------|-------------|------|
| **TeacherRecordsList** | 교사 대시보드 | merits, demerits, monthly | 필터: `teacher_id=eq.${teacherId}` |
| **Student Dashboard** | 학생 대시보드 | merits, demerits, monthly | 필터: `student_id=eq.${studentId}` |
| **PointsInquiry** | 관리자 대시보드 | merits, demerits, monthly | 필터 없음 (모든 데이터) |
| **StudentLeaderboard** | 관리자 대시보드 | merits, demerits, monthly | 필터 없음 + 순위 갱신 |
| **DataInquiry** | 관리자 대시보드 | students, teachers, merits, demerits, monthly | 조건부 새로고침 (dataType 확인) |
| **CounselingInquiry** | 관리자 대시보드 | career_counseling | 단일 테이블 구독 |
| **EmailHistory** | 관리자 대시보드 | email_history | 단일 테이블 구독 |
...
│                          Supabase Database                               │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌───────────────┐  │
│  │ merits   │ │ demerits │ │ monthly  │ │ students │ │career_counsel.│  │
│  └────┬─────┘ └────┬─────┘ └────┬─────┘ └────┬─────┘ └──────┬────────┘  │
│       │            │            │            │              │           │
│       └────────────┴────────────┴────────────┴──────────────┘           │
│                                 │                                        │
...
│   교사 화면      │     │   학생 화면      │     │  관리자 화면     │
│                 │     │                 │     │                 │
│ TeacherRecords  │     │ StudentDashboard│     │ PointsInquiry   │
│ List            │     │                 │     │ StudentLeader   │
│ (본인 기록만)    │     │ (본인 기록만)    │     │ DataInquiry     │
│                 │     │                 │     │ Counseling      │
│                 │     │                 │     │ EmailHistory    │
└─────────────────┘     └─────────────────┘     └─────────────────┘
```
...
### 🎯 목표
반복되는 실시간 동기화 코드를 **재사용 가능한 커스텀 훅**으로 만들어서 코드 중복을 줄입니다.

### 🤔 왜 커스텀 훅이 필요할까요?

위에서 본 것처럼 모든 컴포넌트에 비슷한 코드가 반복됩니다:
1. 페이지 포커스 시 새로고침
2. Supabase 채널 구독
3. 토스트 알림 표시
4. Cleanup 함수

**커스텀 훅**을 사용하면 이 모든 것을 한 곳에서 관리할 수 있습니다!

---

### 💻 커스텀 훅 코드 (use-realtime-sync.ts)
...
export interface TableSubscription {
  table: string;           // 테이블 이름
  channelName: string;     // 채널 고유 이름
  filter?: string;         // 필터 조건 (선택)
  labels?: {               // 토스트 메시지
    insert?: string;
    update?: string;
    delete?: string;
  };
  condition?: () => boolean;  // 새로고침 조건 (선택)
}
...
**쉽게 이해하기:**
- `useRef`: 항상 최신 함수를 참조하기 위한 저장소
- `dependencies`: 값이 바뀌면 구독을 다시 설정
- `condition`: 특정 조건에서만 새로고침

---

### 💻 미리 정의된 테이블 구독 설정

자주 사용하는 테이블 구독은 미리 정의해 둡니다:

```typescript
// 상벌점 테이블 (merits, demerits, monthly)
...
const PointsInquiry = () => {
  ...
  // 커스텀 훅 사용 - 이 한 줄로 실시간 동기화 완료!
  useRealtimeSync({
    tables: POINTS_TABLES.map(t => ({
      ...t,
      channelName: `points_inquiry_${t.table}`,  // 고유한 채널 이름
    })),
...
|------|------------|------------|
| 코드 줄 수 | ~100줄 | ~15줄 |
| 중복 코드 | 각 컴포넌트에 반복 | 커스텀 훅에서 한 번만 |
| 유지보수 | 어려움 (수정 시 여러 파일) | 쉬움 (훅만 수정) |
| 버그 가능성 | 높음 | 낮음 |
...
새로운 컴포넌트에서 `audit_logs` 테이블의 변경을 감지하는 실시간 동기화 코드를 작성해보세요.
...
| **useCallback** | 함수 메모이제이션 | `useCallback(() => {}, [deps])` |

---

## 7. 교사 대시보드 컴포넌트 실시간 동기화 ⭐ 새로운 내용!

### 🎯 목표
교사 대시보드의 StudentGroupManager와 BulkEmailSender 컴포넌트에 useRealtimeSync 커스텀 훅을 적용하여 실시간 동기화를 구현합니다.

---

### 💻 StudentGroupManager 컴포넌트 리팩토링

학생 그룹을 관리하는 컴포넌트에 실시간 동기화를 적용합니다.

#### 변경 전 코드 (수동 구독)

```typescript
// StudentGroupManager.tsx - 변경 전
...
```typescript
// StudentGroupManager.tsx - 변경 후

import { useState, useEffect, useCallback } from "react";
import { useRealtimeSync, TableSubscription } from "@/hooks/use-realtime-sync";

// 컴포넌트 상단에서 사용자 정보 가져오기
const authUser = localStorage.getItem("auth_user");
...
**쉽게 이해하기:**
- 60줄 이상의 코드가 → 20줄 이하로 줄어듭니다!
- 페이지 포커스 감지와 실시간 구독을 모두 커스텀 훅이 처리합니다
- `useCallback`으로 함수를 감싸면 불필요한 재구독을 방지합니다

---

### 💻 BulkEmailSender 컴포넌트 리팩토링

일괄 발송 컴포넌트에서도 그룹 변경을 실시간으로 감지합니다.

#### 변경 전 코드

```typescript
// BulkEmailSender.tsx - 변경 전

// 그룹 목록만 불러오는 useEffect
useEffect(() => {
  loadGroups();
  loadTemplates();
}, []);

// 문제점: 다른 탭에서 그룹을 수정해도 이 컴포넌트는 모릅니다!
```

#### 변경 후 코드

```typescript
// BulkEmailSender.tsx - 변경 후
...
const groupTables: TableSubscription[] = user ? [
  {
    channelName: "bulk-email-groups",  // 고유한 채널 이름
    table: "student_groups",
    filter: `admin_id=eq.${user.id}`,
    labels: {
      insert: "🔄 그룹이 추가되어 목록을 갱신합니다",
      update: "🔄 그룹이 수정되어 목록을 갱신합니다",
      delete: "🔄 그룹이 삭제되어 목록을 갱신합니다",
    },
  },
] : [];
...
**쉽게 이해하기:**
- `channelName`은 각 컴포넌트마다 다르게 설정합니다 (중복 방지)
- StudentGroupManager에서 그룹을 수정하면 → BulkEmailSender에서도 자동 갱신!
- 같은 테이블(`student_groups`)을 구독하지만 채널 이름이 다릅니다

---

### 🖼️ 컴포넌트 간 실시간 동기화 흐름도
...
└─────────────────────────────────────────────────────────────┘
            ...
            ▼                                     ▼
┌────────────────────────┐          ┌────────────────────────┐
│   StudentGroupManager   │          │    BulkEmailSender     │
│ 채널: student-group-    │          │ 채널: bulk-email-      │
│        manager          │          │        groups          │
│                         │          │                         │
│ loadGroups() 실행!      │          │ loadGroups() 실행!     │
│ → 그룹 목록 갱신        │          │ → 드롭다운 목록 갱신   │
└────────────────────────┘          └────────────────────────┘
```
...
|------|---------|---------|
| 코드 줄 수 | ~60줄 | ~20줄 |
| 포커스 감지 | 직접 구현 | 훅이 자동 처리 |
| 실시간 구독 | 직접 구현 | 훅이 자동 처리 |
| 구독 해제 | 직접 구현 | 훅이 자동 처리 |
| 토스트 알림 | 직접 구현 | labels 설정만 |
| 재사용성 | 없음 | 어디서든 재사용 가능 |
...
#### 과제 1: 채널 이름의 중요성
다음 두 컴포넌트가 같은 채널 이름을 사용하면 어떤 문제가 생길까요?

```typescript
// StudentGroupManager
channelName: "group-changes"

// BulkEmailSender
channelName: "group-changes"  // 같은 이름!
```

<details>
<summary>정답 보기</summary>

채널 이름이 같으면 구독 충돌이 발생할 수 있습니다:
1. 한 컴포넌트가 언마운트될 때 채널을 제거하면, 다른 컴포넌트도 영향받을 수 있습니다
2. 각 컴포넌트마다 고유한 채널 이름을 사용해야 안전합니다
3. 예: `student-group-manager`, `bulk-email-groups` 처럼 구분하세요
...
#### 과제 2: 선택적 새로고침 구현
BulkEmailSender에서 그룹이 삭제되었을 때만 선택된 그룹을 초기화하려면?

<details>
<summary>정답 보기</summary>

현재 useRealtimeSync 훅은 모든 이벤트에 동일한 onRefresh를 호출합니다.
이벤트 타입별로 다른 동작이 필요하다면:

1. 훅을 확장하여 이벤트 타입별 콜백을 추가하거나
2. 컴포넌트에서 별도의 실시간 구독을 추가로 구현합니다

```typescript
// 방법 2 예시: 삭제 시 선택 초기화
useEffect(() => {
  const channel = supabase
    .channel('bulk-email-delete-handler')
    .on('postgres_changes', {
      event: 'DELETE',
      schema: 'public',
      table: 'student_groups',
    }, () => {
      setSelectedGroupId("");  // 선택 초기화
    })
    .subscribe();
