...
| 정책명 | 명령 | 조건 | 조건 설명 |
|--------|------|------|----------|
| `Admins can view all monthly` | SELECT | `app.current_admin_id IS NOT NULL` | 관리자는 모든 월별 추천을 볼 수 있어요 |
| `Admins can insert monthly` | INSERT | `app.current_admin_id IS NOT NULL` | 관리자는 월별 추천을 추가할 수 있어요 |
| `Admins can update monthly` | UPDATE | `app.current_admin_id IS NOT NULL` | 관리자는 월별 추천을 수정할 수 있어요 |
| `Admins can delete monthly` | DELETE | `app.current_admin_id IS NOT NULL` | 관리자는 월별 추천을 삭제할 수 있어요 |
| `Teachers can view all monthly` | SELECT | `app.current_teacher_id IS NOT NULL` | 교사는 모든 월별 추천을 볼 수 있어요 |
| `Teachers can view monthly` | SELECT | `app.current_teacher_id OR app.current_admin_id` | 교사/관리자 월별 추천 조회 |
| `Teachers can insert monthly` | INSERT | `teacher_id = app.current_teacher_id` | 교사는 본인 명의로 월별 추천을 할 수 있어요 |
| `Teachers can insert monthly recommendations` | INSERT | `teacher_id = app.current_teacher_id OR is_admin_user()` | 교사/관리자가 월별 추천을 추가할 수 있어요 |
| `Teachers can delete own monthly` | DELETE | `teacher_id = app.current_teacher_id` | 교사는 본인이 추천한 것만 삭제할 수 있어요 |
| `Students can view own monthly` | SELECT | `student_id = app.current_student_id` | 학생은 본인이 받은 월별 추천만 볼 수 있어요 |
